// Specialist centres shown in the "Our Centers" section (home + about).
// NOTE: images for Medical / Pain / Migraine are placeholders using existing
// clinic photos — the client should supply dedicated imagery per centre.
export const centres = [
  {
    slug: 'algarve-spine-center',
    name: 'Algarve Spine Center',
    image: '/assets/SpineCenter.webp',
    body: 'Spine diagnostics, minimally invasive interventions, and surgical pathways coordinated by our team.',
  },
  {
    slug: 'algarve-medical-center',
    name: 'Algarve Medical Center',
    image: '/assets/images/medical/ALGARVEPAIN202062-min.webp',
    body: 'Multidisciplinary consultations with fast access to imaging, rehabilitation, and follow-up care.',
  },
  {
    slug: 'algarve-pain-centre',
    name: 'Algarve Pain Centre',
    image: '/assets/images/medical/APC_OUTUBRO2021170-min.webp',
    body: 'Evidence-based pain medicine with integrated rehabilitation and long-term care planning.',
  },
  {
    slug: 'algarve-migraine-neurotherapy-centre',
    name: 'Algarve Migraine & Neurotherapy Centre',
    image: '/assets/images/Hero/HeadPain.webp',
    body: 'Dedicated migraine and headache care, with neurotherapy pathways and preventive treatment planning.',
    path: '/algarve-migraine-neurotherapy-centre',
  },
];
